...
function renderBacklog(state) {
    ...
    // На мобиле НЕ открываем сами — бэклог там показывается оверлеем по кнопке,
    // иначе он перекрыл бы стол.
    if (hasItems && !backlogOpen && !isMobile()) {
        backlogOpen = true;
    }
...
$("deckCollapseBtn").addEventListener("click", () => {
    const collapsed = $("deckBar").classList.toggle("collapsed");
