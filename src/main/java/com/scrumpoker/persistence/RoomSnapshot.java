...
public record RoomSnapshot(
    ...
    public record BacklogSnap(String id, String title, String estimate, int revotes, List<VoteSnap> votes) {}
    ...
    public static RoomSnapshot from(Room room) {
        ...
        List<BacklogSnap> bl = room.getBacklog().stream()
                .map(i -> new BacklogSnap(
                        i.getId(), i.getTitle(), i.getEstimate(), i.getRevotes(),
                        i.getVotes().stream()
    ...
    public Room toRoom() {
        ...
        if (backlog != null) {
            for (BacklogSnap bs : backlog) {
                ...
                item.setRevotes(bs.revotes());
                if (bs.votes() != null) {
                    item.setVotes(bs.votes().stream()
                            .map(v -> new BacklogItem.VoteRecord(v.name(), v.value()))
                            .collect(Collectors.toList()));
                }
